...
kmeansW.repeat <- function(x, centers, weight = rep(1, nrow(x)), iter.max = 100, nstart = 1,step=20) {
  ...
  for (i in 1:round(nstart/step)) {
    ...
    if (tot < best) {
      best_clus = clus
      best = tot
      flog.info("running weighted k-means [%2i%%] completed ", round(100*i*step/nstart))
    } else {
      flog.info("running weighted k-means [%2i%%] completed <<<<<", round(100*i*step/nstart))
    }
  }
...
#' @export
grouping.getMeasures <- function(sim,y_var="y1",measure="ecdf",Nw=40,model=NA,exclude_movers=FALSE,user_fn=NA,three_way=T) {

  if(three_way){
    sim$jdata$x <- 1
    sdata = rbind(sim$sdata,sim$jdata)
    fids = sdata[,f1]
	print(head(sim$jdata,5))
    jdata_2 = sim$jdata[!(f2 %in% fids)]
    jdata_2 <- jdata_2 %>% mutate(f1 = f2, y1 = y2)
    print(head(jdata_2,10))
    sdata = rbind(sdata,jdata_2)
  }
...
#' @export
grouping.getMeasures.em <- function(sim,y_var="y1",measure="ecdf",Nw=40,model=NA,exclude_movers=FALSE,user_fn=NA,three_way=T) {
  ...
  N = sdata[,length(unique(f1))]
  qs = seq(1/(Nw+1),Nw/(Nw+1),l=Nw)
  flog.info("processing %i unique id's",N)
...
grouping.getMeasures.em.nested <- function(sim,y_var="y1",group_var="f1",measure="ecdf",Nw=40,model=NA,exclude_movers=FALSE,user_fn=NA,three_way=T) {
  ...
  # extract dep variable
  sdata <- sdata[,ydep := get(y_var)]
  sdata <- sdata[,group_id := get(group_var)]

  N = sdata[,length(unique(group_id))]
  qs = seq(1/(Nw+1),Nw/(Nw+1),l=Nw)
  flog.info("processing %i firms/managers",N)
  ...
  if (str_detect(measure,"user_")) {
    ...
    # for each firm get the quantiles
    rs     = sdata[,list(cdf = as.numeric(t.cum(ydep,wv)/.N), s = 1:Nw),group_id]
    M      = acast(rs,group_id~s,value.var = "cdf")/sqrt(Nw)
    #rd     = sdata[, W[f1]*sum(t.discrepancy(ydep,wv,sqrt(Nw)*M[f1,]))/(.N^2),f1][,sum(V1)]/(N*Nw)
  } else if (measure == "ecdf_true") {
    # for each firm get the quantiles
    rs     = sdata[,list(cdf = pnorm(wv,mean= am[1] + psi[1], sd= sqrt( (model$eta_sd_m0 + model$eta_sd_m1*am[1])^2 + model$eps_sd^2)), s = 1:Nw),f1]
  ...
  fids = rownames(M)
  W   = rep(0,length(fids))
  sdata.size = sdata[,.N,group_id]
  sdata.size[,f1s:=paste(group_id)]
  setkey(sdata.size,f1s)
  W = sdata.size[fids,N]
    ...
    if ((sum(kres_min$withinss/N) > cval*measures$discrepency) &
      ...
      for (i in k_min:k_max) {
        ...
        if ( k_mid == k_min) {
          ...
          flog.info("k=%i WSS=%f nstart=%i ids=%i",k,sum(kres$withinss),nstart,N)
